function pagesFun(frameArg,everyPages){
	console.log($(frameArg).length)


  $(frameArg).hide()
for(var index = 0;index<everyPages;index++){
...
var pages=parseInt(($(frameArg).length-1)/everyPages+1);
...
$(".a-firstPages").click(function(){
	...
	$(".a-pagesSure").click(function(){
		 ...
		 i=pagesValue-1;
		for(var index=everyPages*(pagesValue-1);index<everyPages*(pagesValue-1)+everyPages;index++){
		$(frameArg).eq(index).show()
	   }
...
$(".a-next").click(function(){
	...
	for(var index=everyPages*i;index<everyPages*i+everyPages;index++){
		$(frameArg).eq(index).show()
	}
